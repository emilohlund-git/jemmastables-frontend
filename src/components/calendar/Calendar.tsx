        ...
        <div className="flex flex-col py-10">
          ...
          <div className="flex flex-col mx-1 sm:hidden">
            {data?.appointments.map((appointment, idx) => {
              if (
                moment(mobileVisibleDay).format("YYYY-MM-DD") ==
                moment(appointment.date).format("YYYY-MM-DD")
              ) {
                if (
                  !moment(appointment.date).add(1, "days").isBefore(moment())
                ) {
                  return (
                    <>
                      <p
                        key={idx}
                        className="text-black text-2xl font-thin mb-3 ml-3"
                      >
                          ...
                          >
                            <span className="font-bold text-lg">
                              {moment(appointment.from).format("HH:mm") +
                                " till " +
                                moment(appointment.to).format("HH:mm") +
                                " " +
                                appointment.type}
                            </span>
                          </div>
